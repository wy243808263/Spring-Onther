...
<c:forEach var="v" items="${page.list}">
	   <tr id="${v.id}" align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#F1F8B4';" onMouseOut="javascript:this.bgColor='#FFFFFF';">
			...
			<td><div align="center"><a href="#" onclick="toModify('<%=path %>/server/role!findByIdForAllot.action?role.id=${v.id}',350,550);return false;" style="cursor: pointer;">资源</a>&nbsp;<a href="#" onclick="toModify('<%=path %>/server/role!findById.action?role.id=${v.id}',350,550);return false;" style="cursor: pointer;">修改</a>&nbsp;<a href="#" onclick="removeById('<%=path %>/server/role!removeById.action?role.id=${v.id}',${v.id});return false;" style="cursor: pointer;">删除</a></div></td>
		</tr>
</c:forEach>
