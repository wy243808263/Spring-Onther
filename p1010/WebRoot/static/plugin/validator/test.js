jQuery.fn.extend({
	required: function(e){
		if(this.val() == '' || this.val() == null || this.val() == undefine){
			alert(e);
			this.focus();
			return false;
		}
	...
	rangeSize: function(v1, v2){
		if(this.val().length < v1 || this.val().length > v2){
			alert('长度应在' + v1 + '-' + v2 + '之间');
			this.focus();
			return false;
		}
	...
	phone: function(){
		if(!/^([\+][0-9]{1,3}[ \.\-])?([\(]{1}[0-9]{2,6}[\)])?([0-9 \.\-\/]{3,20})((x|ext|extension)[ ]?[0-9]{1,4})?$/.test(this.val())){
			alert('电话号码不合法');
			this.focus();
			return false;
		}
