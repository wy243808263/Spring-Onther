...
function showAdd(){
	location="${ctx}/purchaseunitsController/toadd.do";
}
function showupdate(){
	...
	if(id!=0){
		if(id.indexOf(",")>0){
			...
			location="${ctx}/purchaseunitsController/toedit.do?id="+id;
		}
	}else{
		alert("请选择数据");
	}
...
function delete_list(){
	...
	if(id!=0){
		if(id.indexOf(",")>0){
			if(confirm("是否删除??")){
				window.location.href="${ctx}/purchaseunitsController/delitem.do?ids="+id;
			}
		}else{
			if(confirm("是否删除??")){
				window.location.href="${ctx}/purchaseunitsController/delete.do?id="+id;
			}
		}
	}else{
		alert("请选择数据");
					...
					<div class="navbar-inner">
                        ...
                        <table class="table table-striped table-bordered" id="sample_1">
                            ...
                            <tr>
                                ...
                                <th class="hidden-phone">纳税人识别号</th>
                                <th class="hidden-phone">地址电话</th>
                                <th class="hidden-phone">银行账号</th>
                                <th class="hidden-phone">组织机构代码</th>
                            </tr>
                            </thead>
                            <tbody>
									<c:forEach items="${purchaseunitss}" var="item" varStatus="status">
										<tr class="odd gradeX">
