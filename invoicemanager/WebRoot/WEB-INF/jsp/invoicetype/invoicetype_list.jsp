...
function showAdd(){
	location="${ctx}/invoicetypeController/toadd.do";
}
function showupdate(id){
	if(id==0){
		...
		if(id.indexOf(",")>0){
			...
			location="${ctx}/invoicetypeController/toedit.do?id="+id;
		}
	}else{
		alert("请选择数据");
	}
...
function delete_list(id){
	if(id==0){
		...
		if(id.indexOf(",")>0){
			if(confirm("是否删除??")){
				window.location.href="${ctx}/invoicetypeController/delitem.do?ids="+id;
			}
		}else{
			if(confirm("是否删除??")){
				window.location.href="${ctx}/invoicetypeController/delete.do?id="+id;
			}
		}
	}
}
