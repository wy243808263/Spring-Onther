...
function showAdd(){
	location="${ctx}/customerController/toadd.do";
}
function showupdate(id){
	if(id==0){
		...
		if(id.indexOf(",")>0){
			...
			location="${ctx}/customerController/toedit.do?id="+id;
		}
	}else{
		alert("请选择数据");
	}
...
function delete_list(id){
	if(id==0){
		...
		if(id.indexOf(",")>0){
			if(confirm("是否删除??")){
				window.location.href="${ctx}/customerController/delitem.do?ids="+id;
			}
		}else{
			if(confirm("是否删除??")){
				window.location.href="${ctx}/customerController/delete.do?id="+id;
			}
		}
	}
}
